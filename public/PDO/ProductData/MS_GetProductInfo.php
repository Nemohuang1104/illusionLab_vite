...
try {
    ...
    if ($product) {
        ...
        // 查詢商品的所有樣式 (需要使用 FETCH_ASSOC)
        $styleSql = "SELECT STYLE_VALUE, STYLE_IMG FROM PRODUCT_STYLES WHERE PRODUCT_ID = :productId";
        $styleStmt = $pdo->prepare($styleSql);
        $styleStmt->bindParam(':productId', $productId, PDO::PARAM_INT);
        $styleStmt->execute();
        $styles = $styleStmt->fetchAll(PDO::FETCH_ASSOC);  // 改為 FETCH_ASSOC
